...
public class Exer2Test {
	...
	public void testInsert() {
		...
		int insertCount = update(sql,type,ID,examCard,studentName,location,grade);
		if (insertCount > 0) {
	...
	//通用的增删改操作
		public int update(String sql,Object ...agrs) {//sql中占位符的个数与可变形参的长度相同！
			...
			try {
				...
				for (int i = 0; i < agrs.length; i++) {
					ps.setObject(i + 1, agrs[i]);//小心参数声明错误！！
				}
				//4.执行
				/*
				 * ps.execute()：
